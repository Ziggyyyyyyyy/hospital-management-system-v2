...
import PostVisitNoteForm from './post-visit-note-form'
        ...
        {isLoading ? (
          ...
          <Tabs defaultValue="personal" className="w-full">
            ...
            <TabsContent value="consultation" className="space-y-4 mt-4">
              {patient.appointment_id || patient.record_id ? (
                <PostVisitNoteForm
                  appointment_id={patient.appointment_id || patient.record_id || ''}
                  onSuccess={() => onOpenChange(false)}
                />
              ) : (
                <div className="text-sm text-muted-foreground p-4 bg-muted/20 rounded-md">
                  No active appointment ID associated with this record to write a post-visit note.
                </div>
              )}
            </TabsContent>
          </Tabs>
