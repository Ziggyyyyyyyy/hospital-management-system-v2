...
export default function DispenseButton({
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      setRecords(Array.isArray(data) ? data : data.appointments || [])
    } catch (err: any) {
  ...
  const handleDispenseMedicine = async () => {
    ...
    const selectedMedicine = medicines.find(
      (medicine) => String(medicine.medicine_id) === String(medicine_id),
    )
  ...
  const handleMedicineSelection = (value: string) => {
    ...
    const selected = medicines.find((med) => String(med.medicine_id) === String(value))
    setSelectedMedicineDetails(selected || null)
  }

  const handleRecordSelection = (value: string) => {
    setFormData((prev) => ({ ...prev, record_id: value }))
    const selected = records.find((rec) => String(rec.record_id) === String(value))
    setSelectedRecordDetails(selected || null)
  }

  const filteredRecords = (records || []).filter((record) => {
    const firstName = record.patients?.users?.first_name || ''
    const lastName = record.patients?.users?.last_name || ''
